...
```
```{r}
plot_lm_us <- plot_builder(data_clean, "United States", "#E76BF3")
plot_lm_australia <- plot_builder(data_clean, "United States", "#F8766D")
plot_lm_germany <- plot_builder(data_clean, "Germany", "#A3A500")
plot_lm_singapore <- plot_builder(data_clean, "Singapore", "#00B0F6")
plot_lm_japan <- plot_builder(data_clean, "Japan", "#00BF7D")
...
SimDistribution <- function(lm, bin, poslim, neglim, country){
...
new_p <- sim_response %>% 
  ...
  labs(title=glue(country, " Observed Distribution"), x = "Simulated Murder Rate per 100K",
       ...
       subtitle = "Count") +
  theme_bw()+
  scale_x_continuous(limits=c(neglim,poslim))
  
return(new_p+obs_p)

}
SimDistribution(totalcountry_lm, 1, "Overall")
SimDistribution(us_lm, 0.2, "US")
SimDistribution(australia_lm, 0.2, "Australia")
```

```{r}
#3.2 NOT FINISHED
#Feel free to refine/fix
