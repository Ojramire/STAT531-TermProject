...
The SimDistribution function, which uses the linear models created for each country, the datasets created for each country, and a choosen bin size, was used to plot each of the histograms shown above. A noise function, which randomly generates noise using an input (x), mean, and standard deviation (sd), was then used to create the simulated histograms for each country within the SimDistribution function, and plot those as well. The simulated and observed data for each country, as well as overall, are presented side by side in Figure 6. These represent the simulated and observed murder rate per 100K for each country.
