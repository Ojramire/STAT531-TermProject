...
something about the countries selected

```{r}
data_clean <- data_opt |>
  ungroup() |>
  select(year:"Sustainable Development Index") |>
  remove_missing() |>
  group_by(year) |>
  summarise("Average Murder Rate" = mean(`Murder Rate per 100K`),
            "Average SDI"         = mean(`Sustainable Development Index`))
...
The analysis for this data set will be performed with a single regression analysis for all 61 countries combined. In the following graphs, Figure 1 captures the overall average SDI versus average murder rate for all countries, while Figure 2 plots both variables separately over time for the countries.

```{r}
data_clean |>
  ggplot(aes(x = `Average SDI`, y = `Average Murder Rate`)) +
  geom_point(aes(color = year)) +
  labs(x = "Average Sustainable Development Index (SDI)/Year", 
       y = "", subtitle = "Average Murder Rate per 100K/Year", 
       caption = "Figure 1. Relationship between he average yearly SDI and Murder Rate per 100k for all 61 selected countries.") +
...
SDI_plot + 
  murder_plot + 
  plot_annotation(caption = "Figure 2. Average SDI and Average Murder Rate per 100K from 1989 to 2006.",
