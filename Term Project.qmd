...
```{r}
#| fig-height: 20
resid_total <- regfunction(lm_totalcountry, 8, "Overall")
resid_us <- regfunction(lm_us, 0.75, "United States")
resid_australia <- regfunction(lm_australia, 0.5, "Australia")
resid_germany <- regfunction(lm_germany, 0.3, "Germany")
resid_singapore <- regfunction(lm_singapore, 0.5, "Singapore")
resid_japan <- regfunction(lm_japan, 0.2, "Japan")
...
SimDistribution <- function(lm, data, bin, countryname){
...
obs_p <- data %>% 
  ...
  labs(title=glue(countryname, " Simulated Distribution"), x = "Observed Murder Rate per 100K",
       y = "",
       subtitle = "Count") +
  theme_bw()+
...
new_p <- sim_response %>% 
  ggplot(aes(x = sim_MR)) +
  geom_histogram(binwidth = bin) +
  labs(title=glue(countryname, " Observed Distribution"), x = "Simulated Murder Rate per 100K",
       y = "",
       subtitle = "Count") +
  theme_bw()+
  scale_x_continuous(limits=c(-10, 15))
...
SimDistribution(lm_totalcountry, data_clean, 1, "Overall")
SimDistribution(lm_us, data_us, 1, "United States")
SimDistribution(lm_australia, data_australia, 1, "Australia")
SimDistribution(lm_germany, data_germany, 1, "Germany")
SimDistribution(lm_singapore, data_singapore, 1, "Singapore")
SimDistribution(lm_japan, data_japan, 1, "Japan")
```

Shown above are two histograms for each country, representing the simulated murder rate per 100K and observed murder rate per 100K for each country. For the overall observed versus overall simulated histograms, we can see that there is a much wider spread in the simulated data, with the mean centered around 3, multiple peaks at 1, 3, and 5, and a slight left skew. The observed data for overall countries has a mean that is much more concentrated around 1, and few data points around 6-10, creating a right skew. 

For each individual countries simulated distribution histograms, we see a much smaller spread of data, with the US having a mean centered around 6 and right skew, Australia having a mean centered around 2, and Germany, Singapore, and Japan having their mean's at 1. These individual histograms help explain why the overall simulated histogram has peaks at 1, 3, and 5.

The observed data for each of these countries and the overall countries all appear the same? 
...
us_sims
sim_r_sq <- us_sims %>%  
  map(~ lm(`Murder Rate per 100K` ~ .x, data = us_sims)) %>%  
  map(glance) %>%  
  map_dbl(~ .x$r.squared)
...
tibble(us_sims = sim_r_sq) %>%  
  ggplot(aes(x = us_sims)) + 
