...
SimDistribution <- function(lm, bin, country){
...
obs_p <- data_clean %>% 
  ...
  labs(title=glue(country, " Simulated Distribution"), x = "Observed Murder Rate per 100K",
       y = "",
       subtitle = "Count") +
  theme_bw()+
...
new_p <- sim_response %>% 
  ggplot(aes(x = sim_MR)) +
  geom_histogram(binwidth = bin) +
  labs(title=glue(country, " Observed Distribution"), x = "Simulated Murder Rate per 100K",
       y = "",
       subtitle = "Count") +
  theme_bw()+
  scale_x_continuous(limits=c(-10, 15))
...
SimDistribution(lm_totalcountry, 1, "Overall")
SimDistribution(lm_us, 1, "US")
SimDistribution(lm_australia, 1, "Australia")
SimDistribution(lm_germany, 1, "Germany")
SimDistribution(lm_singapore, 1, "Singapore")
SimDistribution(lm_japan, 1, "Japan")
```
