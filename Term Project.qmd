...
data_clean |>
  ...
  geom_point(aes(color = fct_reorder(country, `Murder Rate per 100K`, .desc = TRUE, .na_rm = TRUE))) +
...
SDI_plot <- data_clean |>
  ...
  geom_line(aes(color = fct_reorder(country, `Murder Rate per 100K`, .desc = TRUE, .na_rm = TRUE))) +
  labs(x = "Year", 
       y = "", 
...
SimDistribution(lm_us, 0.2, "US")
SimDistribution(lm_australia, 0.2, "Australia")
```

```{r}
#3.2 Done in Basic way according to textbook
#Feel free to refine/fix/functionize
